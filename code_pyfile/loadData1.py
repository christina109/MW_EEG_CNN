...
def load_dataset(sub, task, feat, conds, nBack, norm = False, sidx = 'all'):

    sessions = [1,2]
    for ss in sessions:
        beh = get_beh(sub, task, nBack, [ss])
...
def load_dataset_feats(sub, task, feats, conds, nBack, norm = False, sidx = 'all'):
# only works when combining power and ispc, since they share the same freq/pnt (1,2) dimensions

    for fi in range(len(feats)):
        feat = feats[fi]
        f_sidx = sidx[fi]
        x, y = load_dataset(sub, task, feat, conds, nBack, norm, f_sidx)
...
def load_dataset_n(subs, task, feats, conds, nBack, norm = False, sidx = 'all'):

    s = []
    for sub in tqdm(subs):
        x, y = load_dataset_feats(sub, task, feats, conds, nBack, norm, sidx)
...
def load_dataset_n_tasks(subs, tasks, feats, conds, winlen, norm = False, sidx='all'):

    for task in tasks:
        nBack = get_nBack(task, winlen)
        x, y, s = load_dataset_n(subs, task, feats, conds, nBack, norm, sidx)
        if task == tasks[0]:
            xs = x.copy()
            ys = y.copy()
            ss = s.copy()
...
def load_dataset_n_split(subs, task, feats, conds, nBack, norm = False, testSize = 0.2, randSeed = None, sidx = 'all'):
...
def get_class_size(subs, tasks, conds, winlen):
    x,y,s = load_dataset_n_tasks(subs, tasks, ['raw'], conds, winlen, norm=False, sidx='all')# load the smallest feature for counting trials
    return (Counter(y))
