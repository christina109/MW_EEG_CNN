...
# randomization
randseed = 87  # [36, 54, 87]  # 27 during testing phase

# data and feature type
study = 1
feats = ['raw'] # [['raw'], ['power', 'ispc'], ['wst']]
conds = ['ot', 'mw']
tasks = ['sart', 'vs']  # dual tasks for variability (default)


aspOn = False  # across-task predictions default: False
#asp_task = ['sart']  # task for test, only valid when aspOn
subs2rm = [1,4,9,12,14,16,17,19,24,25,28,30]
...
#hpar = 'idv_models'
# hvals = range(1,31) # default
#hvals = [2,3,4,5,6,7,8,10,14,15,18,20,21,22,23,26,27,29]  # study 1 published
...
def modelling(hpar, hval, x_train, y_train, x_test, y_test, lnpoi):
    ...
    x = Conv1D(neurons[0], 3, activation = 'relu', kernel_regularizer = reg, name = 'block1_conv1')(inputs)
    #x = Conv1D(neurons[0], 3, activation = 'relu', kernel_regularizer = reg, name = 'block1_conv2')(x)
    x = MaxPooling1D(pool_size=2, name = 'block1_pool')(x)

    x = Conv1D(neurons[1], 3, activation = 'relu', kernel_regularizer = reg, name = 'block2_conv1')(x)
    #x = Conv1D(neurons[1], 3, activation = 'relu', kernel_regularizer = reg, name = 'block2_conv2')(x)
    x = MaxPooling1D(pool_size=2, name = 'block2_pool')(x)

    if False:
        x = Conv1D(neurons[2], 3, activation = 'relu', kernel_regularizer = reg, name = 'block3_conv1')(x)
        x = Conv1D(neurons[2], 3, activation = 'relu', kernel_regularizer = reg, name = 'block3_conv2')(x)
        x = Conv1D(neurons[2], 3, activation = 'relu', kernel_regularizer = reg, name = 'block3_conv3')(x)
        x = MaxPooling1D(pool_size=2, name = 'block3_pool')(x)

        x = Conv1D(neurons[3], 3, activation = 'relu', kernel_regularizer = reg, name = 'block4_conv1')(x)
        x = Conv1D(neurons[3], 3, activation = 'relu', kernel_regularizer = reg, name = 'block4_conv2')(x)
        x = Conv1D(neurons[3], 3, activation = 'relu', kernel_regularizer = reg, name = 'block4_conv3')(x)
...
def lnpocv(norm, winlen=20, sidx='all', subs=subs):
    ...
    for lop in ifelse(hpar == 'leaveout_participant', hvals, [0]):
        ...
        for lnpoi in tqdm(ifelse(aspOn, range(1), range(int(1/lnpo)))):
            ...
            if not hpar == 'idv_models':
                ...
                if not aspOn:
                    ...
                    x_test, y_test, junk = ld1.load_dataset_n_tasks(subs, asp_task, feats, conds, winlen, norm, sidx)
