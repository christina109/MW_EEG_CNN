...
def load_dataset(sub, task, feat, conds, nBacks, norm = False, sidx = 'all',
    ...
    for condi in range(len(conds)):
        ...
        if isinstance(nBacks, list):
            beh = get_beh(sub, task, nBacks[condi])
        else:
            beh = get_beh(sub, task, nBacks)
...
def load_dataset_n_split(subs, task, feats, conds, nBack, norm = False, testSize = 0.2, randSeed = None, sidx = 'all', gpu2use = -1):

    for sub in tqdm(subs):
        x, y = load_dataset_feats(sub, task, feats, conds, nBack, norm, sidx, gpu2use)
