...
def load_dataset(sub, task, feat, conds, nBack, norm = False, sidx = 'all'):
    ...
    for condi in range(len(conds)):
        ...
        for triali in range(ntrial):
            ...
            trialname = triallist.iloc[triali]
            trialname = str(int(trialname)).zfill(4)
            f_load = os.path.join('.', 'feats_matfile','2', str(sub).zfill(3), trialname, feat+ '.mat')
