...
def get_beh(sub, task, nBack = 0, session = [1,2]):  # nBack == 0 - load all
    ...
    if nBack == 0:
        if notifyOn: print('Load all behavioral data.')
    elif nBack > 0:
        if notifyOn: print('Load data from previous %s trials.' %nBack)
